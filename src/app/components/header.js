        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 xl:px-12 py-3 lg:py-5">
            ...
            <nav className="hidden lg:flex flex-1 justify-center">
              <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl inline-flex items-center px-2 xl:px-8 py-2">
                {menuItems.map((item, index) => (
                  <div key={item.href} className="flex items-center">
                    {item.isDropdown ? (
                      <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                        <Link href={item.href} className="flex items-center gap-2 text-gray-300 hover:text-white transition-all duration-300 font-semibold text-sm xl:text-base group px-3 xl:px-4 py-2 hover:bg-white/5 rounded-xl">
                          <span>{item.label}</span>
                          <svg className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${isTiendaOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                        </Link>
                        {isTiendaOpen && (
                    ...
                    {index < menuItems.length - 1 && (<div className="w-px h-4 xl:h-5 bg-white/20 mx-1 xl:mx-2"></div>)}
                  </div>
                ))}
              </div>
            </nav>
                    ...
                    {item.isDropdown ? (
                      <div>
                        <button 
                          ...
                          className={`w-full flex items-center justify-center text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-300 py-4 px-6 font-semibold text-base uppercase tracking-wide relative ${index < menuItems.length - 1 ? 'border-b border-white/10' : ''}`}
                        >
                        ...
                        <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isMobileTiendaOpen ? 'max-h-96' : 'max-h-0'}`}>
                          ...
                          <div className="bg-black/30 border-y border-white/10">
                            {tiendaCategories.map(subItem => (
                              <Link key={subItem.href} href={subItem.href} className="block text-gray-200 hover:text-white hover:bg-white/5 py-3 px-6 font-medium text-base text-center" onClick={() => setIsMenuOpen(false)}>
                                {subItem.label}
                              </Link>
                            ))}
                          </div>
                      ...
                      <a href={item.href} className={`flex items-center justify-center text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-300 py-4 px-6 font-semibold text-base uppercase tracking-wide ${index < menuItems.length - 1 ? 'border-b border-white/10' : ''}`} onClick={() => setIsMenuOpen(false)}>
                        {item.label}
                      </a>
                    )}
                  </div>
