...
export default function CartModal({ showCart, setShowCart }) {
  ...
  const generateOrderPDF = async () => {
    ...
    if (!billingData.email || !billingData.email.includes('@')) {
      alert('Por favor ingresa un email válido antes de generar el PDF');
      return;
    }
    ...
    // ENVÍO AUTOMÁTICO
    // ENVÍO AUTOMÁTICO con validación
      try {
        const pdfBase64 = doc.output('datauristring');

        console.log('Email a enviar:', billingData.email); // Para debug
        ...
        const response = await fetch('/api/sendOrderEmail', {
          ...
          body: JSON.stringify({
            ...
            pdfBase64,
            filename: `pedido_${new Date().toISOString().split('T')[0]}_${billingData.lastName}.pdf`,
          }),
        });

        if (response.ok) {
          alert('PDF generado y enviado por correo electrónico exitosamente.');
        } else {
          const error = await response.json();
          console.error('Error enviando email:', error);
          alert('PDF generado, pero hubo un error enviando el email: ' + error.error);
        }
      } catch (error) {
        console.error('Error en el envío:', error);
        alert('PDF generado, pero hubo un error enviando el email.');
      }
  };
